...
class ListHandler:
    ...
    def get_list(self, list_id: UUID4 = Query(None, description='Shopping list ID')):
        session = db.session
        try:
            lst = session.query(List).filter(List.list_id == list_id).one()
        except NoResultFound:
            raise HTTPException(404, "List not found")
        return lst

    @router.post('/{list_id}:share', response_model=ListGet, status_code=status.HTTP_200_OK)
    def share_list(self, list_id: UUID4 = Query(None, description='Shopping list ID')):
        session = db.session
        try:
            lst = session.query(List).filter(List.list_id == list_id).one()
        except NoResultFound:
    ...
    def mark_bought(self, list_id: UUID4 = Query(None, description='Shopping list ID')):
        session = db.session
        try:
            lst = session.query(List).filter(List.list_id == list_id).one()
        except NoResultFound:
